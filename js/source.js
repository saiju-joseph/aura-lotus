   ...
   function recalcCart() {
      ...
      document.querySelectorAll('.cart-card').forEach((card) => {
         ...
         const finalPrice = parseFloat(
            wrapper.dataset.finalPrice || wrapper.dataset.price || 0
         );
      ...
      if (discountEl)
         discountEl.textContent = 'AED ' + totalDiscount.toFixed(2);
   ...
   sendOtpBtn.addEventListener('click', async function () {
      ...
      if (!phone) {
         otpMessage.innerHTML =
            "<div class='text-danger'>Enter phone number</div>";
         return;
      }
...
if (verifyOtpBtn) {
   verifyOtpBtn.addEventListener('click', async function () {
      ...
      if (otp.length !== 6) {
         otpMessage.innerHTML =
            "<div class='text-danger'>Enter valid OTP</div>";
         return;
      }
      ...
      if (data.status === 'verified') {
         ...
         otpMessage.innerHTML = "<div class='text-danger'>Invalid OTP</div>";
      }
   });
}
document.addEventListener('DOMContentLoaded', function () {
